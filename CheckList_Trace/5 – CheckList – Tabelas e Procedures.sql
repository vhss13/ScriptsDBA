...
--	Criacao das tabelas que serao utilizadas para gerar o relatario do CheckList em HTML
...
/*******************************************************************************************************************************
--	Database que sera utilizada para armazenar os dados do CheckList. Se for necessario, altere o nome da mesma.
*******************************************************************************************************************************/
use Traces

GO
/*******************************************************************************************************************************
--	Criacao das tabelas para armazenar os dados do CheckList
*******************************************************************************************************************************/
...
/*
OBJETIVO: Procedure responsavel por retirar os caracteres invalidos para o XML.
...
/*******************************************************************************************************************************
-- Criacao das procedures para popular as tabelas criadas acima:
--	1) Espaao em Disco
...
--	10) Contadores
--	11) Fragmentacao de indices
--	12) Waits Stats
--	13) Error Log SQL
*******************************************************************************************************************************/

-- Libera permissoes para pegar informacoes de acesso a disco com a proc sp_OACreate
...
BEGIN
	...
	BEGIN
		INSERT INTO [dbo].[CheckList_Espaco_Disco]( [DriveName], [TotalSize_GB], [FreeSpace_GB], [SpaceUsed_GB], [SpaceUsed_Percent] )
		SELECT 'Sem registro de Espaco em Disco', NULL, NULL, NULL, NULL
	END
END
...
BEGIN
	...
	-- COLETA DE INFORMACOES SOBRE ARQUIVOS MDF
...
BEGIN
	...
	-- Declara e seta AS variaveis das datas - Tratamento para os casos que ainda nao atingiram 60 dias no historico
...
BEGIN
	...
	-- Declara e seta AS variaveis das datas - Tratamento para os casos que ainda nao atingiram 60 dias no histarico
	DECLARE @Dt_Hoje DATE, @Dt_1Dia DATE, @Dt_15Dias DATE, @Dt_30Dias DATE, @Dt_60Dias DATE
...
BEGIN
	...
	BEGIN
		INSERT INTO [dbo].[CheckList_Utilizacao_Arquivo_Writes]
		SELECT 'Sem registro de Utilizacao dos Arquivos - Writes', NULL, NULL, NULL, NULL
	END
	...
	BEGIN
		INSERT INTO [dbo].[CheckList_Utilizacao_Arquivo_Reads]
		SELECT 'Sem registro de Utilizacao dos Arquivos - Reads', NULL, NULL, NULL, NULL
	END
END
...
BEGIN
	...
	-- Verifica as databases sem backup nas ultimas 16 horas
	...
	BEGIN
		INSERT INTO [dbo].[CheckList_Databases_Sem_Backup] ( Nm_Database )
		SELECT 'Sem registro de Databases Sem Backup nas ultimas 16 horas.'
	END
END
...
BEGIN
	...
	BEGIN
		INSERT INTO [dbo].[CheckList_Backups_Executados] (	[Database_Name], [Name], [Backup_Start_Date], [Tempo_Min], [Position], [Server_Name],
															[Recovery_Model], [Logical_Device_Name], [Device_Type], [Type], [Tamanho_MB] )
		SELECT 'Sem registro de Backup FULL ou Diferencial nas ultimas 24 horas.', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL
	END
END
GO
...
BEGIN
	...
	SET [sql_command] = REPLACE( REPLACE( REPLACE( REPLACE( CAST([sql_command] AS VARCHAR(1000)), '<aquery --', ''), '--a>', ''), '&gt;', '>'), '&lt;', '')

	-- Exclui os registros das queries com menos de 2 horas de execucao
...
BEGIN
	...
	WHERE	ja.session_id = (SELECT TOP 1 session_id FROM msdb.dbo.syssessions ORDER BY agent_start_date DESC)
			...
			AND DATEDIFF(minute,start_execution_date, GETDATE()) >= 10		-- No minimo 10 minutos em execucao

	IF (@@ROWCOUNT = 0)
	BEGIN
		INSERT INTO [dbo].[CheckList_Jobs_Running] (Nm_JOB, Dt_Inicio, Qt_Duracao, Nm_Step)
		SELECT 'Sem JOBs em execucao a mais de 10 minutos', NULL, NULL, NULL
	END	
END
...
BEGIN
	...
	BEGIN
		INSERT INTO [dbo].[CheckList_Conexao_Aberta_Email] ([Nr_Ordem], [login_name], [session_count])
		SELECT NULL, 'Sem conexoes de usuarios abertas', NULL
	END
END
...
BEGIN
	...
	BEGIN
		INSERT INTO [dbo].[CheckList_Fragmentacao_Indices] (	[Dt_Referencia], [Nm_Servidor], [Nm_Database], [Nm_Tabela], [Nm_Indice], 
																[Avg_Fragmentation_In_Percent], [Page_Count], [Fill_Factor], [Fl_Compressao] )
		SELECT NULL, NULL, 'Sem registro de indice com mais de 10% de Fragmentacao', NULL, NULL, NULL, NULL, NULL, NULL
	END
END
...
/*******************************************************************************************************************************
--	Tabela de controle que sera utilizada para armazenar o Historico dos Alertas.
*******************************************************************************************************************************/
...
BEGIN
	...
	-- Busca os Alertas a partir da Data de Referencia
	...
	-- Busca os Alertas que estao sem o CLEAR
