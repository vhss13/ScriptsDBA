...
*******************************************************************************************************************************/
--	1) Substituir o e-mail "E-mail@provedor.com" pelos emails que devem receber os Alertas e substituir o profile 'MSSQLServer' pelo que sera utilizado

DECLARE @Ds_Email VARCHAR(MAX)
DECLARE @Ds_Profile_Email VARCHAR(MAX)

SELECT @Ds_Email = 'vhss13@gmail.com'
SELECT @Ds_Profile_Email = 'MSSQLServer'
--	2) Basta aperta F5 para executar o script completo.


/*******************************************************************************************************************************
--	Database que sera utilizada para armazenar os dados dos Alertas. Se for necessario, altere para o nome desejado.
*******************************************************************************************************************************/
USE [DBA_MON]

--------------------------------------------------------------------------------------------------------------------------------
--	Criacao das tabelas de Controle dos Alertas
--------------------------------------------------------------------------------------------------------------------------------
use [DBA_MON]
...
--------------------------------------------------------------------------------------------------------------------------------
--	Insere os dados na tabela de Parametro
--------------------------------------------------------------------------------------------------------------------------------
INSERT INTO [dbo].[Alerta_Parametro]([Nm_Alerta], [Nm_Procedure], [Fl_Clear], [Vl_Parametro], [Ds_Metrica], [Ds_Profile_Email], [Ds_Email]) 
VALUES	('Versao CheckList Banco de Dados',	'1.0.0',									0,		NULL,	NULL,			@Ds_Profile_Email,	@Ds_Email),
		('Versao Alerta',					'1.0.0',									0,		NULL,	NULL,			NULL,				NULL),
		...
		('Conexao SQL Server',				'stpAlerta_Conexao_SQLServer',				1,		2000,	'Quantidade',	@Ds_Profile_Email,	@Ds_Email),
		('Status Database',					'stpAlerta_Erro_Banco_Dados',				1,		NULL,	NULL,			@Ds_Profile_Email,	@Ds_Email),
		('Pagina Corrompida',				'stpAlerta_Erro_Banco_Dados',				0,		NULL,	NULL,			@Ds_Profile_Email,	@Ds_Email),
		...
		('Processos em Execucao',			'stpEnvia_Email_Processos_Execucao',		0,		NULL,	NULL,			@Ds_Profile_Email,	@Ds_Email)

-- Verifica o resultado da tabela com os parametros dos Alertas			
select * from [DBA_MON]..Alerta_Parametro
