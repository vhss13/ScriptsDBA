...
/*******************************************************************************************************************************
--	Tabela de controle que sera utilizada para armazenar o Historico da tabela Suspect Pages.
*******************************************************************************************************************************/
...
BEGIN
	...
	-- Processo Bloqueado
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Processo Bloqueado')
	...
	--------------------------------------------------------------------------------------------------------------------------------
	-- Retorna todos os processos que estao sendo executados no momento
	...
	-- Verifica se nao existe nenhum processo em Execucao
	...
	-- Verifica o ultimo Tipo do Alerta registrado -> 0: CLEAR / 1: ALERTA
	...
	*******************************************************************************************************************************/	
	IF EXISTS	(
					...
					WHERE	DATEDIFF(SECOND,A.[start_time], @Dt_Atual) > @Processo_Bloqueado_Parametro * 60		-- A query que esta sendo bloqueada esta rodando a mais 2 minutos
							AND DATEDIFF(SECOND,B.[start_time], @Dt_Atual) > @Qt_Tempo_Raiz_Lock * 60			-- A query que esta bloqueando esta rodando a mais de 1 minuto
				)
	BEGIN	-- INICIO - ALERTA
		...
		BEGIN
			...
			-- Tratamento quando nao tem um Lock Raiz
			...
			*******************************************************************************************************************************/							
			...
			SET @AlertaLockRaizTable = 
					...
					+	'<tr>
							...
							<th bgcolor=#0B0B61 width="200"><font color=white>Hora Inicio</font></th>
							<th bgcolor=#0B0B61 width="120"><font color=white>Status</font></th>
							<th bgcolor=#0B0B61 width="30"><font color=white>ID Sessao</font></th>
							<th bgcolor=#0B0B61 width="60"><font color=white>ID Sessao Bloqueando</font></th>
							<th bgcolor=#0B0B61 width="120"><font color=white>Wait</font></th>
							<th bgcolor=#0B0B61 width="60"><font color=white>Transacoes Abertas</font></th>
			...
			--------------------------------------------------------------------------------------------------------------------------------
			-- Insere um Espaco em Branco no EMAIL
			--------------------------------------------------------------------------------------------------------------------------------
			SET @EmptyBodyEmail =	''
			SET @EmptyBodyEmail =
			...
			*******************************************************************************************************************************/			              
			...
			/*******************************************************************************************************************************
			-- Inclui uma imagem com link para o site
			*******************************************************************************************************************************/
			select @EmailBody = @EmailBody + '<br/><br/>' +
				'<a href="http://www.pointsystems.com.br/" target=”_blank”> 
					<img src="http://www.pointsystems.com.br/images/Logo%20Point.png" height="100" width="400"/></a>'
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			...
			SET @AlertaLockTable = CAST( (
				...
				FROM (  
						-- Dados da Tabela do EMAIL
						SELECT	ISNULL([dd hh:mm:ss.mss], '-')							AS [Duracao], 
								ISNULL([database_name], '-')							AS [database_name],
								ISNULL([login_name], '-')								AS [login_name],
								ISNULL([host_name], '-')								AS [host_name],
								ISNULL(CONVERT(VARCHAR(20), [start_time], 120), '-')	AS [start_time],
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: Nao existe mais algum Processo Bloqueado a mais de ' + 
...
BEGIN
	...
	-- Arquivo de Log Full
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Arquivo de Log Full')
	...
	/*******************************************************************************************************************************
	-- Verifica se existe algum LOG com muita utilizacao
	*******************************************************************************************************************************/
	IF EXISTS(
				SELECT	*
	...
	BEGIN	-- INICIO - ALERTA
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			...
			SET @AlertaLogHeader = @AlertaLogHeader + '<BR /> Informacoes dos Arquivos de Log <BR />'
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'ALERTA: Existe algum Arquivo de Log com mais de ' +  CAST((@Log_Full_Parametro) AS VARCHAR) + '% de utilizacao no Servidor: ' + @@SERVERNAME,
	...
	BEGIN	-- INICIO - CLEAR
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: Nao existe mais algum Arquivo de Log com mais de ' +  CAST((@Log_Full_Parametro) AS VARCHAR) + '% de utilizacao no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaLogHeader + @EmptyBodyEmail + @AlertaLogTable + @EmptyBodyEmail + 
									@ResultadoWhoisactiveHeader + @EmptyBodyEmail + @ResultadoWhoisactiveTable + @EmptyBodyEmail
...
BEGIN
	...
	-- Cria as tabelas que irao armazenar as Informacoes do Espaco em Disco	
	...
	-- Popula as tabelas com as Informacoes sobre o Espaco em Disco
	...
	-- Utiliza o Cursor para gerar as Informacoes de cada Disco
	...
	-- Tabela com os dados resumidos sobre o Espaco em Disco
	...
	--------------------------------------------------------------------------------------------------------------------------------
	-- Espaco Disco
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Espaco Disco')
	...
	/*******************************************************************************************************************************
	--	Verifica o Espaco Livre em Disco
	*******************************************************************************************************************************/
	IF EXISTS	(		
					SELECT NULL 
	...
	BEGIN	-- INICIO - ALERTA
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			...
			SET @AlertaDiscoHeader = @AlertaDiscoHeader + '<BR /> Espaco em Disco no Servidor <BR />' 
			...
			SET @AlertaDiscoTable = CAST( (    
				...
				FROM (   
						...
						FROM [DBA_MON].[dbo].[_DTS_Espacodisco]
			...
			*******************************************************************************************************************************/				              
			SELECT	@Importance =	'High',
					@Subject =		'ALERTA: Existe algum volume de disco com mais de ' +  CAST((@Espaco_Disco_Parametro) AS VARCHAR) + '% de utilizacao no Servidor: ' + @@SERVERNAME,
	...
	BEGIN	-- INICIO - CLEAR				
		...
		BEGIN
			...
			*******************************************************************************************************************************/				              
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: Nao existe mais algum volume de disco com mais de ' +  CAST((@Espaco_Disco_Parametro) AS VARCHAR) + '% de utilizacao no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaDiscoHeader + @EmptyBodyEmail + @AlertaDiscoTable + @EmptyBodyEmail +
									@ResultadoWhoisactiveHeader + @EmptyBodyEmail + @ResultadoWhoisactiveTable + @EmptyBodyEmail
...
BEGIN
	...
	-- Consumo CPU
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Consumo CPU')
	...
	/*******************************************************************************************************************************
	--	Verifica se o Consumo de CPU esta maior do que o parametro
	*******************************************************************************************************************************/
	IF (
	...
	BEGIN	-- INICIO - ALERTA	
		...
		BEGIN
			...
			BEGIN
				...
				*******************************************************************************************************************************/
				...
				SET @AlertaCPUAgarradosTable = 
						...
						+	'<tr>
								...
								<th bgcolor=#0B0B61 width="200"><font color=white>utilizacao Total (%)</font></th>
							</tr>'    
				...
				*******************************************************************************************************************************/			              
				SELECT	@Importance =	'High',
						@Subject =		'ALERTA: O Consumo de CPU esta acima de ' +  CAST((@CPU_Parametro) AS VARCHAR) + '% no Servidor: ' + @@SERVERNAME,
	...
	BEGIN	-- INICIO - CLEAR		
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: O Consumo de CPU esta abaixo de ' +  CAST((@CPU_Parametro) AS VARCHAR) + '% no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaCPUAgarradosHeader + @EmptyBodyEmail + @AlertaCPUAgarradosTable + @EmptyBodyEmail +
									@ResultadoWhoisactiveHeader + @EmptyBodyEmail + @ResultadoWhoisactiveTable + @EmptyBodyEmail
...
BEGIN
	...
	-- MaxSize Arquivo SQL
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'MaxSize Arquivo SQL')

	-- Cria as tabelas que irao armazenar as Informacoes sobre os arquivos
	...
	-- Busca as Informacoes sobre os arquivos LDF
	...
	/*******************************************************************************************************************************
	--	Verifica se existe algum arquivo MDF ou LDF proximo do MaxSize
	*******************************************************************************************************************************/
	IF EXISTS	(
					SELECT NULL
	...
	BEGIN	-- INICIO - ALERTA		
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			...
			SET @AlertaMDFLDFHeader = @AlertaMDFLDFHeader + '<BR /> Informacoes arquivos .LDF e .MDF com "MaxSize" especificado <BR />' 
			...
			SET @AlertaMDFLDFTable = CAST( (    
				...
				FROM (  
						...
						SELECT	DISTINCT 
								...
								[Growth]
						FROM [DBA_MON].[dbo].[_Resultado_Alerta_SQLFile]
						WHERE	(
									CASE WHEN [MaxSize(MB)] >= 150000 
			...
			SET @AlertaMDFLDFTable = 
					...
					+	'<tr>
							...
							<th bgcolor=#0B0B61 width="100"><font color=white>Nome Logico</font></th>  
	...
	BEGIN	-- INICIO - CLEAR			
		...
		BEGIN
			...
			*******************************************************************************************************************************/					              
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: Nao existe mais algum arquivo MDF ou LDF com risco de estouro do Maxsize no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaMDFLDFHeader + @EmptyBodyEmail + @AlertaMDFLDFTable + @EmptyBodyEmail
...
BEGIN
	...
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Tempdb Utilizacao Arquivo MDF')
	...
	-- Conexoes mais antigas que 1 hora
	...
	-- Busca as Informacoes do Tempdb
	...
	-- Busca as Transacoes que estao abertas
	...
	-- Query Alerta Tempdb - Conexoes abertas - Incluir no Alerta TempDb
	...
	-- Tratamento caso Nao retorne nenhuma conexao
	...
	/*******************************************************************************************************************************
	--	Verifica se o Consumo do Arquivo do Tempdb esta muito grande
	*******************************************************************************************************************************/
	IF EXISTS	(
	...
	BEGIN	-- INICIO - ALERTA				
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			...
			SET @AlertaTamanhoMDFTempdbTable = 
					...
					+	'<tr>
							...
							<th bgcolor=#0B0B61 width="200"><font color=white>Espaco Reservado (MB)</font></th>
							<th bgcolor=#0B0B61 width="200"><font color=white>Percentual Utilizado (%)</font></th>
							<th bgcolor=#0B0B61 width="200"><font color=white>Espaco Nao Alocado (MB)</font></th>
							<th bgcolor=#0B0B61 width="200"><font color=white>Espaco Objetos Internos (MB)</font></th>
							<th bgcolor=#0B0B61 width="200"><font color=white>Espaco Version Store (MB)</font></th>
							<th bgcolor=#0B0B61 width="200"><font color=white>Espaco Objetos de Usuario (MB)</font></th>
						</tr>'    
			...
			*******************************************************************************************************************************/
			...
			SET @AlertaTempdbUtilizacaoArquivoHeader = @AlertaTempdbUtilizacaoArquivoHeader + '<BR /> TOP 50 - Conexoes com Transacao Aberta <BR />' 
			...
			*******************************************************************************************************************************/			              
			SELECT	@Importance =	'High',
					@Subject =		'ALERTA: O Tamanho do Arquivo MDF do Tempdb esta acima de 70% no Servidor: ' + @@SERVERNAME,
	...
	BEGIN	-- INICIO - CLEAR		
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: O Tamanho do Arquivo MDF do Tempdb esta abaixo de 70% no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaTamanhoMDFTempdbHeader + @EmptyBodyEmail + @AlertaTamanhoMDFTempdbTable + @EmptyBodyEmail +
									@AlertaTempdbUtilizacaoArquivoHeader + @EmptyBodyEmail + @AlertaTamanhoMDFTempdbConexoesTable + @EmptyBodyEmail+
									@AlertaTempdbProcessoExecHeader + @EmptyBodyEmail + @AlertaTempdbProcessoExecTable + @EmptyBodyEmail
...
BEGIN
	...
	-- Conexoes SQL Server
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'conexao SQL Server')
	...
	SELECT	@Conexoes_SQLServer_Parametro = Vl_Parametro,	-- Quantidade
			...
			@ProfileEmail = Ds_Profile_Email
	FROM [dbo].[Alerta_Parametro]
	WHERE [Id_Alerta_Parametro] = @Id_Alerta_Parametro		-- Conexoes SQL Server
	...
	/*******************************************************************************************************************************
	--	Verifica se o limite de Conexoes para o Alerta foi atingido
	*******************************************************************************************************************************/
	...
	BEGIN	-- INICIO - ALERTA		
		...
		*******************************************************************************************************************************/
		...
		--------------------------------------------------------------------------------------------------------------------------------
		--	ALERTA - Conexoes - HEADER
		--------------------------------------------------------------------------------------------------------------------------------
		SET @AlertaConexaoSQLServerHeader = '<font color=black bold=true size=5>'				            
		SET @AlertaConexaoSQLServerHeader = @AlertaConexaoSQLServerHeader + '<BR /> TOP 25 - Conexoes Abertas no SQL Server <BR />' 
		SET @AlertaConexaoSQLServerHeader = @AlertaConexaoSQLServerHeader + '</font>'
		
		--------------------------------------------------------------------------------------------------------------------------------
		--	ALERTA - Conexoes - BODY 
		--------------------------------------------------------------------------------------------------------------------------------
		...
		SELECT	TOP 25 IDENTITY(INT, 1, 1) AS id, 
				...
				case when es.[program_name] = '' then 'Sem nome na string de conexao' else [program_name] end [program_name], 
		...
		SET @AlertaConexaoSQLServerTable = 
				...
				+	'<tr>
						...
						<th bgcolor=#0B0B61 width="10"><font color=white>Qtd. Conexoes</font></th>
					</tr>'    
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: Existem ' + cast(@Qtd_Conexoes as varchar) + ' Conexoes Abertas no SQL Server no Servidor: ' + @@SERVERNAME,
	...
	BEGIN	-- INICIO - CLEAR		
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			...
			--------------------------------------------------------------------------------------------------------------------------------
			SET @AlertaConexaoSQLServerHeader = '<font color=black bold=true size=5>'				            
			SET @AlertaConexaoSQLServerHeader = @AlertaConexaoSQLServerHeader + '<BR /> Conexoes abertas no SQL Server <BR />' 
			SET @AlertaConexaoSQLServerHeader = @AlertaConexaoSQLServerHeader + '</font>'
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: Existem ' + cast(@Qtd_Conexoes as varchar) + ' Conexoes Abertas no SQL Server no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaConexaoSQLServerHeader + @EmptyBodyEmail + @AlertaConexaoSQLServerTable + @EmptyBodyEmail+ 
									@AlertaConexaoProcessosExecHeader + @EmptyBodyEmail + @AlertaConexaoProcessosExecTable + @EmptyBodyEmail
...
BEGIN
	...
	-- Pagina Corrompida
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Pagina Corrompida')
	...
	*******************************************************************************************************************************/
	...
	--------------------------------------------------------------------------------------------------------------------------------
	-- Status Database
	SELECT @Id_Alerta_Parametro = 8	-- SELECT * FROM [DBA_MON].[dbo].Alerta_Parametro
	
	SELECT	@EmailDestination = Ds_Email,
			@ProfileEmail = Ds_Profile_Email
	FROM [dbo].[Alerta_Parametro]
	WHERE [Id_Alerta_Parametro] = @Id_Alerta_Parametro		-- Pagina Corrompida

	/*******************************************************************************************************************************
	-- Verifica se existe alguma Pagina Corrompida
	*******************************************************************************************************************************/
	...
	BEGIN	-- INICIO - ALERTA	
		...
		*******************************************************************************************************************************/			
		...
		SET @AlertaPaginaCorrompidaHeader = @AlertaPaginaCorrompidaHeader + '<BR /> Paginas Corrompidas <BR />' 
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: Existe alguma Pagina Corrompida no Banco de Dados no Servidor: ' + @@SERVERNAME,
	...
	*******************************************************************************************************************************/	
	-- Status Database
	SELECT @Id_Alerta_Parametro = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Status Database')
	...
	/*******************************************************************************************************************************
	-- Verifica se alguma Database Nao esta ONLINE
	*******************************************************************************************************************************/ 
	IF EXISTS	(
					SELECT NULL
					FROM [sys].[databases]
	...
	BEGIN	-- INICIO - ALERTA		
		...
		BEGIN			
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'ALERTA: Existe alguma Database que Nao esta ONLINE no Servidor: ' + @@SERVERNAME,
	...
	BEGIN	-- INICIO - CLEAR			
		...
		BEGIN
			...
			*******************************************************************************************************************************/
			SELECT	@Importance =	'High',
					@Subject =		'CLEAR: Nao existe mais alguma Database que Nao esta ONLINE no Servidor: ' + @@SERVERNAME,
					@EmailBody =	@AlertaStatusDatabasesHeader + @EmptyBodyEmail + @AlertaStatusDatabasesTable + @EmptyBodyEmail 
...
BEGIN
	...
	-- Queries Demoradas
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Queries Demoradas')
	...
	/*******************************************************************************************************************************
	--	Verifica se existem mais de 100 Queries Lentas nos ultimos 5 minutos
	*******************************************************************************************************************************/
	...
	BEGIN
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: Existem ' + CAST(@Quantidade_Queries_Demoradas AS VARCHAR) + ' queries demoradas nos ultimos 5 minutos no Servidor: ' + @@SERVERNAME,
...
BEGIN
	...
	-- Job Falha
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Job Falha')
	...
	WHERE 
		 -- [Step_Id] = 0 AND condicao para o retry
		  [Run_Status] <> 1 AND
		  CAST	(	
	...
	*******************************************************************************************************************************/
	...
	BEGIN
		...
		*******************************************************************************************************************************/
		...
		--------------------------------------------------------------------------------------------------------------------------------
		SET @AlertaLogHeader = '<font color=black bold=true size=5>'				            
		SET @AlertaLogHeader = @AlertaLogHeader + '<BR /> TOP 50 - Jobs que Falharam nas ultimas ' +  CAST((@JobFailed_Parametro) AS VARCHAR) + ' Horas <BR />' 
		SET @AlertaLogHeader = @AlertaLogHeader + '</font>'
		...
		SET @AlertaJobFailed = 
				...
				+	'<tr>
						...
						<th bgcolor=#0B0B61 width="150"><font color=white>Data da Execucao</font></th>
						<th bgcolor=#0B0B61 width="100"><font color=white>Duracao</font></th>
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: Jobs que Falharam nas ultimas ' +  CAST((@JobFailed_Parametro) AS VARCHAR) + ' Horas no Servidor: ' + @@SERVERNAME,
...
BEGIN
	...
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'SQL Server Reiniciado')
	...
	*******************************************************************************************************************************/
	...
	BEGIN
		...
		*******************************************************************************************************************************/
		...
		--------------------------------------------------------------------------------------------------------------------------------
		SET @AlertaLogHeader = '<font color=black bold=true size= 5>'
		SET @AlertaLogHeader = @AlertaLogHeader + '<BR /> SQL Server Reiniciado nos ultimos ' +  CAST((@SQL_Reiniciado_Parametro) AS VARCHAR) + ' Minutos <BR />'
		SET @AlertaLogHeader = @AlertaLogHeader + '</font>'
		...
		SET @AlertaSQLReiniciado =	
				'<table cellspacing="2" cellpadding="5" border="3">'
				+	'<tr>
						<th width="400" bgcolor=#0B0B61><font color=white>Horario Restart</font></th>
					 </tr>'
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: SQL Server Reiniciado nos ultimos ' +  CAST((@SQL_Reiniciado_Parametro) AS VARCHAR) + ' Minutos no Servidor: ' + @@SERVERNAME,
...
BEGIN
	...
	-- Database Criada
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Database Criada')
	...
	*******************************************************************************************************************************/
	...
	BEGIN
		...
		*******************************************************************************************************************************/
		...
		--------------------------------------------------------------------------------------------------------------------------------
		SET @AlertaLogHeader = '<font color=black bold=true size= 5>'
		SET @AlertaLogHeader = @AlertaLogHeader + '<BR /> Database Criada nas ultimas ' +  CAST((@Database_Criada_Parametro) AS VARCHAR) + ' Horas <BR />'
		SET @AlertaLogHeader = @AlertaLogHeader + '</font>'
		...
		SET @AlertaBaseCriada =	
				...
				+	'<tr>
						...
						<th width="300" bgcolor=#0B0B61><font color=white>Data Criacao</font></th>
					 </tr>'
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: Database Criada nas ultimas ' +  CAST((@Database_Criada_Parametro) AS VARCHAR) + ' Horas no Servidor: ' + @@SERVERNAME,
...
BEGIN
	...
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Database sem Backup')
	...
	*******************************************************************************************************************************/
	...
	BEGIN	
		-- Databases que Nao tiveram Backup
		...
		*******************************************************************************************************************************/
		...
		--------------------------------------------------------------------------------------------------------------------------------
		SET @AlertaLogHeader = '<font color=black bold=true size=5>'				            
		SET @AlertaLogHeader = @AlertaLogHeader + '<BR /> Databases sem Backup nas ultimas ' +  CAST((@Database_Sem_Backup_Parametro) AS VARCHAR) + ' Horas <BR />' 
		SET @AlertaLogHeader = @AlertaLogHeader + '</font>'
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: Existem Databases sem Backup nas ultimas ' +  CAST((@Database_Sem_Backup_Parametro) AS VARCHAR) + ' Horas no Servidor: ' + @@SERVERNAME,
				@EmailBody =	@AlertaLogHeader + @EmptyBodyEmail + @AlertaLogTable + @EmptyBodyEmail
...
BEGIN
	...
	-- Declara a tabela que ira armazenar o nome das Databases
...
BEGIN
	...
	-- Seleciona o numero de arquivos.
	INSERT INTO #logF  
	...
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Banco de Dados Corrompido')
	...
	*******************************************************************************************************************************/
	IF EXISTS (SELECT NULL FROM [DBA_MON].[dbo].[_Result_Corrupcao]) 
	BEGIN	-- INICIO - ALERTA
		/*******************************************************************************************************************************
		--	CRIA O EMAIL - ALERTA
		*******************************************************************************************************************************/			
		...
		SET @AlertaBancoCorrompidoTable = CAST( (    
			...
			FROM (
					...
					SELECT	CONVERT(VARCHAR(20), [LogDate], 120) AS [LogDate],
							...
							[Erros],
							[Text]
					FROM [DBA_MON].[dbo].[_Result_Corrupcao]
		...
		SET @AlertaBancoCorrompidoTable = 
				...
				+	'<tr>
						...
						<th bgcolor=#0B0B61 width="300"><font color=white>Descricao</font></th>
					</tr>'    
		...
		*******************************************************************************************************************************/
		SELECT	@Importance =	'High',
				@Subject =		'ALERTA: Existe algum Banco de Dados Corrompido no Servidor: ' + @@SERVERNAME + '. Verifique com urgencia!',
...
BEGIN
	...
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [DBA_MON].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Processos em Execucao')
	
	SELECT	@EmailDestination = Ds_Email,
			@ProfileEmail = Ds_Profile_Email
	FROM [dbo].[Alerta_Parametro]
	WHERE [Id_Alerta_Parametro] = @Id_Alerta_Parametro		-- Processos em Execucao
	...
	*******************************************************************************************************************************/							
	...
	--------------------------------------------------------------------------------------------------------------------------------
	SET @ResultadoWhoisactiveHeader = '<font color=black bold=true size=5>'			            
	SET @ResultadoWhoisactiveHeader = @ResultadoWhoisactiveHeader + '<BR /> Processos em Execucao no Banco de Dados <BR />'
	SET @ResultadoWhoisactiveHeader = @ResultadoWhoisactiveHeader + '</font>'
	...
	*******************************************************************************************************************************/
	SELECT	@Importance =	'High',
			@Subject =		'Processos em Execucao no Servidor: ' + @@SERVERNAME,
			@EmailBody =	@ResultadoWhoisactiveHeader + @EmptyBodyEmail + @ResultadoWhoisactiveTable + @EmptyBodyEmail
